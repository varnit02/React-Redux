...
class Game extends Component {
  ...
  handleNumberChange = e => {
    ...
    if (!isNaN(guess)) {
      this.setState(() => ({ guess: parseInt(guess, 10) }));
    }
  };
  outcomeClass() {
                ...
                <div className="form-group">
                  <ul className="nav nav-pills nav-fill">
                    <li className="nav-item">
                      <button
                        className={`nav-link ${this.easyLinkClass()}`}
                    ...
                    <li className="nav-item">
                      <button
                        className={`nav-link ${this.mediumLinkClass()}`}
                        onClick={this.startMediumGame}
                      >
                        MEDIUM
                      </button>
                    </li>
                    <li className="nav-item">
                      <button
                        className={`nav-link ${this.hardLinkClass()}`}
